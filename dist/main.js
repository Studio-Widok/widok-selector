...
var Select = /*#__PURE__*/function () {
  function Select(obj, id) {
    ...
    this.expanded = false;
    this.selected = 0;
    this.options = this.obj.find('.select-option').map(function (index, element) {
      return new Option(element, _this, index);
    });
    ...
    this.adjuster = $(document.createElement('div')).addClass('select-adjuster');
    this.obj.prepend(this.adjuster);
    this.sumH = 0;
    this.timeouts = [];
    this.obj.find('.arrow-down').on('click', function () {
      if (!_this.expanded) {
        _this.expand();
      }
    });
    this.options.map(function (index, element) {
      if (element.obj.hasClass('checked')) {
        _this.select(element.id);
      }
    });
  }
  ...
  _createClass(Select, [{
    ...
    value: function expand() {
      ...
      this.timeouts[0] = setTimeout(function () {
        $(document).on('click.f3Selector' + _this2.id, function () {
          return _this2.shrink();
        });
      }, 300);
      this.expanded = true;
    }
    ...
    value: function shrink() {
      ...
      this.timeouts[1] = setTimeout(function () {
        _this3.obj.removeClass('expanded');

        $(document).off('.f3Selector' + _this3.id);
      }, 300);
      this.adjust();
      this.expanded = false;
    }
...
var Option = /*#__PURE__*/function () {
  function Option(obj, parent, id) {
    ...
    this.obj.on('click', function () {
      return _this4.papa.select(_this4.id);
    });
  }
...
function createSelector(selector) {
  var select = new Select(selector, selects.length);
  selects.push(select);
  return select;
}
